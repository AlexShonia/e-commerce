 ...
 */ (function (e) {
	...
	function r(T) {
		...
		if (F !== M) {
			T[0] = F;
			e: for (var Q = 0, se = T.length, Se = se >>> 1; Q < Se; ) {
				var fe = 2 * (Q + 1) - 1,
					ce = T[fe],
					re = fe + 1,
					nt = T[re];
	...
	function j(T, M) {
		...
		try {
			...
			if (f !== null) var Se = !0;
				...
				var fe = n(u);
				fe !== null && K(C, fe.startTime - M), (Se = !1);
			}
			return Se;
		} finally {
			(f = null), (h = F), (m = !1);
...
function kf(e, t) {
	switch ((Oe(_s, t), Oe(Ts, e), Oe(Un, Zs), (e = t.nodeType), e)) {
		case 9:
		case 11:
...
function Sg(e, t, n) {
	...
	if ((Oe(Ie, r), !(t.mode & 1))) t.memoizedState = null;
	else
		switch (o) {
			case "forwards":
...
function o1(e, t, n) {
	switch (t.tag) {
		...
		case 10:
			...
			Oe(yl, r._currentValue), (r._currentValue = o);
			break;
		case 13:
			if (((r = t.memoizedState), r !== null))
				return r.dehydrated !== null
					? (Oe(Ie, Ie.current & 1), (t.flags |= 128), null)
...
function i1(e, t, n) {
	...
	switch ((Ef(t), t.tag)) {
			...
			if (((n = t.type), e !== null && t.stateNode != null))
				...
				if (((e = oo(Un.current)), Ra(t))) {
					...
					switch (
						...
						case "video":
						case "audio":
							for (o = 0; o < ns.length; o++) be(ns[o], r);
							break;
						case "source":
							be("error", r);
							break;
					...
					e: {
						switch (((s = Hc(n, r)), n)) {
							...
							case "video":
							case "audio":
								for (o = 0; o < ns.length; o++) be(ns[o], e);
								o = r;
								break;
							case "source":
								be("error", e), (o = r);
		...
		case 17:
			return bt(t.type) && pl(), ht(t), null;
		case 19:
			if ((Pe(Ie), (i = t.memoizedState), i === null)) return ht(t), null;
							...
							if (((s = wl(e)), s !== null)) {
								for (
									...
									(i = n),
										...
										(n = n.sibling);
								return Oe(Ie, (Ie.current & 1) | 2), t.child;
							}
							e = e.sibling;
						}
					i.tail !== null &&
...
function xo(e) {
	...
	var n = on.transition,
		r = xe;
	try {
		if (((on.transition = null), (xe = 1), e)) return e();
	} finally {
		(xe = r), (on.transition = n), (ye = t), !(ye & 6) && Wr();
	}
}
function zf() {
	(Mt = Go.current), Pe(Go);
...
function eo(e, t, n) {
	var r = xe,
		o = on.transition;
	try {
		(on.transition = null), (xe = 1), h1(e, t, n, r);
	} finally {
		(on.transition = o), (xe = r);
	}
	return null;
}
function h1(e, t, n, r) {
...
function ni() {
	if (jr !== null) {
		var e = py(Rl),
			...
			n = xe;
		try {
			if (((on.transition = null), (xe = 16 > e ? 16 : e), jr === null))
				var r = !1;
			else {
...
const cx = p.forwardRef(({ defaultActiveIndex: e = 0, ...t }, n) => {
	...
	const mn = Le(() => {
			!document.hidden && hb(At.current) && (T ? ge() : Be());
		}),
		le = F === "next" ? "start" : "end";
	sb(() => {
	...
	const Qn = `${K}-item-${F}`,
		...
		vn = p.useCallback(
			(te) => {
				if (w && !/input|textarea/i.test(te.target.tagName))
					switch (te.key) {
						case "ArrowLeft":
							te.preventDefault(), T ? Be(te) : ge(te);
							return;
						case "ArrowRight":
							te.preventDefault(), T ? ge(te) : Be(te);
							return;
					}
				S == null || S(te);
			},
		...
		ko = p.useCallback(
			(te) => {
				if (j) {
					const we = rt.current;
					Math.abs(we) > pb && (we > 0 ? ge(te) : Be(te));
				}
				g === "hover" &&
					yn.set(() => {
						Se(!1);
	...
	p.useEffect(() => {
		...
		return (
			(No.current = window.setInterval(
				...
				(te = (we = Nn.current) != null ? we : y) != null ? te : void 0
			)),
			() => {
							...
							"button",
							{
								...
								"aria-label":
									u != null && u.length
										? u[we]
										: `Slide ${we + 1}`,
								className: we === re ? "active" : void 0,
								onClick: Mi ? Mi[we] : void 0,
								"aria-current": we === re,
							},
							we
						)
	...
	if (E) {
		if (i) {
			var B,
				...
				se = x === pi ? -R[Y] : -O[Y],
				Se = t.elements.arrow,
				fe = m && Se ? dp(Se) : { width: 0, height: 0 },
				...
				Nn = v
					...
					: se + qe + nt + N.mainAxis,
				ge = t.elements.arrow && ra(t.elements.arrow),
				Be = ge
					? C === "y"
						? ge.clientTop || 0
						: ge.clientLeft || 0
...
const hw = p.forwardRef((e, t) => {
	...
	p.useImperativeHandle(t, () => T, [T]),
		mu && !F && n && (Se.current = gc(Y == null ? void 0 : Y.document)),
		n && Q && se(!1);
	const fe = Le(() => {
			if (
				(T.add(),
		...
		ce = Le(() => {
			if (
				...
				var le;
				(le = Se.current) == null || le.focus == null || le.focus(x),
					(Se.current = null);
			}
		});
	p.useEffect(() => {
...
function Rp(e) {
	...
	function qe() {
		...
		return (
			E.initialized || le(je.Pop, E.location, { initialHydration: !0 }), j
		);
	}
	function Gt() {
		d && d(),
	...
	function Be(b, P, k) {
		...
		let G = P.loaderData
			...
			he = E.blockers;
		he.size > 0 &&
			((he = new Map(he)), he.forEach((Ce, ot) => he.set(ot, Mo)));
		let Ye =
			R === !0 ||
			(E.navigation.formMethod != null &&
				En(E.navigation.formMethod) &&
		...
		let ae;
		if (O === je.Pop) {
			let Ce = L.get(E.location.pathname);
			Ce && Ce.has(b.pathname)
			...
			Ce
				? Ce.add(b.pathname)
				: ((Ce = new Set([b.pathname])),
				  L.set(E.location.pathname, Ce)),
				(ae = { currentLocation: E.location, nextLocation: b });
		}
		ge(
			Te({}, P, {
	...
	async function Qn(b, P, k, U, W) {
		W === void 0 && (W = {}), Vn();
		let de = sT(P, k);
		ge({ navigation: de }, { flushSync: W.flushSync === !0 });
		let ne,
			Z = Vd(U, P);
	...
	async function Tn(b, P, k, U, W, de, ne, Z, G, he, Ye) {
		...
		if (
			(Lu(
				(Ee) =>
					!(k && k.some((Ne) => Ne.route.id === Ee)) ||
					(Yt && Yt.some((Ne) => Ne.route.id === Ee))
			),
			(M = ++T),
			...
			return (
				Be(
					...
					Te(
						...
						he ? { actionData: he } : {},
						Ee ? { fetchers: new Map(E.fetchers) } : {}
					),
					{ flushSync: G }
				),
				{ shortCircuited: !0 }
		...
		Ze.forEach((Ee) => {
			K.has(Ee.key) && gn(Ee.key),
				Ee.controller && K.set(Ee.key, Ee.controller);
		});
		let To = () => Ze.forEach((Ee) => gn(Ee.key));
		...
		D && D.signal.removeEventListener("abort", To),
			Ze.forEach((Ee) => K.delete(Ee.key));
		let Jr = sv(Iu);
		if (Jr) {
			if (Jr.idx >= Yt.length) {
				let Ee = Ze[Jr.idx - Yt.length].key;
				Q.add(Ee);
			}
			return (
	...
	async function Ai(b, P, k, U, W, de, ne) {
		...
		let Ce = E.navigation.location || E.location,
			ot = Gi(e.history, Ce, G.signal),
			Yt = a || s,
			Ze =
		...
		let { loaderData: fa, errors: Ee } = rv(
			E,
			E.matches,
			_o,
			Mu,
		...
		te(To),
			E.navigation.state === "loading" && To > M
				...
				: (ge({
						errors: Ee,
						loaderData: ov(E.loaderData, fa, Ze, Ee),
						fetchers: new Map(E.fetchers),
				  }),
				  (J = !1));
	}
	...
	function Xr(b) {
		return (
			u.v7_fetcherPersist &&
				(Se.set(b, (Se.get(b) || 0) + 1), fe.has(b) && fe.delete(b)),
			E.fetchers.get(b) || Vw
		);
	}
	function qn(b) {
	...
	function ko(b) {
		if (u.v7_fetcherPersist) {
			let P = (Se.get(b) || 0) - 1;
			P <= 0 ? (Se.delete(b), fe.add(b)) : Se.set(b, P);
		} else qn(b);
		ge({ fetchers: new Map(E.fetchers) });
	}
	function gn(b) {
			...
			children: c.jsxs(xu, {
				children: [
					c.jsx(ct.LinkContainer, {
						to: "/",
						children: c.jsx(Ia.Brand, { children: "SportShop" }),
					}),
...
Qe.default = Qe;
const ke = Qe.create({ baseURL: "http://localhost:8000" });
function UD({ product: e }) {
	return c.jsxs(An, {
		...
		children: [
			c.jsx(xt, {
				to: `/product/${e._id}`,
				children: c.jsx(An.Img, { src: "" + e.image }),
			}),
			c.jsxs(An.Body, {
				children: [
					c.jsx(xt, {
...
function BD() {
	const {
		...
		"productsTop",
		async () => (await ke.get("/api/products/top/")).data,
		{
			onError: (r) => {
...
function zD() {
	const e = It(),
		...
		r = t.get("page"),
		{
			...
			"products",
			async () =>
				(await ke.get(`/api/products?keyword=${n}&page=${r}`)).data
		);
	return (
		p.useEffect(() => {
			o && e(l0(o.products)), a();
		...
		c.jsxs("div", {
			children: [
				!n && c.jsx(BD, {}),
				c.jsx("h1", { children: "All Products" }),
				i
					? c.jsx(He, {})
					: s
					? c.jsx(ve, {
...
function HD() {
	...
	const [e, t] = p.useState(1),
		...
		l = Je((v) => v.userLogin.userInfo),
		{
			...
			"productDetails",
			async () => (await ke.get(`/api/products/${s}`)).data
		),
		m = It(),
		y = un(
			async () =>
...
function KD() {
	const e = It(),
		...
		o = Ve(),
		{
			...
			if (t) return (await ke.get(`/api/products/${t}`)).data;
		});
	p.useEffect(() => {
		if (i && r) {
			const m = { ...i, qty: r };
...
function VD() {
	...
	const d = Ve(),
		...
		{ error: w, isLoading: S } = cn(
			...
			async () =>
				(
					await ke.get("/api/users/profile/", {
						headers: {
							"Content-type": "application/json",
		...
		{ error: g, isLoading: x } = cn(
			...
			async () =>
				(
					await ke.get("/api/orders/myorders/", {
						headers: {
							"Content-type": "application/json",
							Authorization: `Bearer ${h.token}`,
						},
...
function mL() {
	const e = It(),
		...
		u = un(
			async () =>
				(
					await ke.delete(`/api/users/delete/${a}/`, {
						headers: {
							"Content-type": "application/json",
							Authorization: `Bearer ${r.token}`,
						},
...
function yL() {
	const e = It(),
		...
		[i, s] = p.useState(),
		{
			...
			async () =>
				(
					await ke.get(`/api/products?page=${r}`, {
		...
		f = un(
			async () =>
				(
					await ke.delete(`/api/products/delete/${i}/`, {
						headers: {
							"Content-type": "application/json",
...
function gL() {
	...
	async function O(R) {
		const D = R.target.files[0],
			N = new FormData();
		N.append("image", D), N.append("product_id", S), w(!0);
		try {
			const L = { headers: { "Content-Type": "multipart/form-data" } },
				{ data: $ } = await ke.post("api/products/upload/", N, L);
			i($), w(!1);
		} catch (L) {
			console.log("error uploading file: ", L), w(!1);
		}
	}
	return (
		p.useEffect(() => {
