...
Qe.default = Qe;
const be = Qe.create({ baseURL: "http://localhost:8000" });
...
function BD() {
	const {
		...
		"productsTop",
		async () => (await be.get("/api/products/top/")).data,
		{
			onError: (r) => {
...
function zD() {
	const e = It(),
		...
		r = t.get("page"),
		{
			...
			"products",
			async () =>
				(await be.get(`/api/products?keyword=${n}&page=${r}`)).data
		);
	return (
		p.useEffect(() => {
			o && e(l0(o.products)), a();
...
function HD() {
	...
	const [e, t] = p.useState(1),
		...
		l = Je((v) => v.userLogin.userInfo),
		{
			...
			"productDetails",
			async () => (await be.get(`/api/products/${s}`)).data
		),
		m = It(),
		y = un(
			async () =>
...
function KD() {
	const e = It(),
		...
		o = Ve(),
		{
			...
			if (t) return (await be.get(`/api/products/${t}`)).data;
		});
	p.useEffect(() => {
		if (i && r) {
			const m = { ...i, qty: r };
...
function VD() {
	...
	const d = Ve(),
		...
		{ error: w, isLoading: S } = cn(
			...
			async () =>
				(
					await be.get("/api/users/profile/", {
						headers: {
							"Content-type": "application/json",
		...
		{ error: g, isLoading: x } = cn(
			...
			async () =>
				(
					await be.get("/api/orders/myorders/", {
						headers: {
							"Content-type": "application/json",
							Authorization: `Bearer ${h.token}`,
						},
...
function mL() {
	const e = It(),
		...
		u = un(
			async () =>
				(
					await be.delete(`/api/users/delete/${a}/`, {
						headers: {
							"Content-type": "application/json",
							Authorization: `Bearer ${r.token}`,
						},
...
function yL() {
	const e = It(),
		...
		[i, s] = p.useState(),
		{
			...
			async () =>
				(
					await be.get(`/api/products?page=${r}`, {
		...
		f = un(
			async () =>
				(
					await be.delete(`/api/products/delete/${i}/`, {
						headers: {
							"Content-type": "application/json",
...
function gL() {
	...
	async function O(R) {
		...
		const D = R.target.files[0],
			N = new FormData();
		console.log(be.defaults.baseURL),
			N.append("image", D),
			N.append("product_id", S),
			w(!0);
		try {
			const L = { headers: { "Content-Type": "multipart/form-data" } },
				{ data: $ } = await be.post("api/products/upload/", N, L);
			i($), w(!1);
		} catch {
			w(!1);
		}
